...
const Particles = ({ config }) => {
  ...
  useFrame((state) => {
    ...
    const dragFactor = merged.cursor.dragFactor * 5.0; // Increased for snappier follow
...
const Medusae = ({ className, config, style }) => {
  ...
  return (
    <div
      ...
      style={{ ...style, position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', overflow: 'hidden' }}
    >
      <Canvas
        ...
        performance={{ min: 0.5 }}
      >
        {merged.background.color !== "transparent" && (
